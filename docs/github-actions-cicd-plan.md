...
2026-03-13 時点で、次は実装済みです。

- `.github/workflows/ci.yml`
- `.github/workflows/nightly-compat.yml`
- `.github/workflows/release.yml`
- `.github/workflows/reusable-server-check.yml`
- `.github/workflows/reusable-godot-smoke.yml`
...
このため、現行の CD は `GitHub Release asset 生成` と `publishable package 契約の検証` までは自動化し、`npm publish` は repository variable + npm organization + npm trusted publisher 設定前提で `v*` tag push 時にだけ動く job として置いています。
...
1. `GitHub Release`
   - すべての正式配布物の集約地点
   - `addon zip`, `npm pack tarball`, `SHA256SUMS`, `smoke logs`, `test-reports` を添付する
2. `npm`
   - `@godot-loop-mcp/server` を publish 対象にする
   - publish は trusted publishing 設定完了後に `v*` tag push で自動実行する
...
現行実装では `reusable-server-check.yml` と `reusable-godot-smoke.yml` に切り出し済みです。  
`ci`, `nightly-compat`, `release` は共通の Node/Godot セットアップをこの reusable workflow 経由で実行します。
...
- PR の主線は `bridge-smoke` と `verification-smoke` に寄せ、専用 `observation-integration` は現時点では未追加
...
- `verification-smoke` と release `test-reports` asset は workflow へ接続済み
...
- `gut-adapter` 将来
- `telemetry-smoke` 将来
- `screenshot-capability-check` 将来
...
- `verification-smoke` で `npm --prefix packages/server run smoke:m4` を PR / release workflow に統合済み
- release では `test-reports-$tag.zip` を asset 化する
...
- `packages/server/package.json` の `private: true` を解除済み
- `repository`, `files`, `bin` の契約を明記済み
- lockfile を追加済み
- `npm pack --dry-run` / `npm publish --dry-run` が通る形に固定済み
...
- provenance を有効にする
- 現行 workflow では `v*` tag push と `repository variable` を満たした時だけ動かす
...
この repo が private のまま運用される場合、required reviewers や wait timer に依存しすぎず、`protected tags` を基準に運用します。
...
- `packages/server/package.json` を publishable package に更新
- `bin/godot-loop-mcp-server.js` を追加
- `npm pack --dry-run` と `npm publish --dry-run` を CI に追加
...
- `test-reports` を release asset 化
...
- reusable workflow で `server-check` / `bridge-smoke` / `verification-smoke` を共通化
- `verification-smoke` と `test-reports` asset を追加
...
- PR で `server-check`, `bridge-smoke`, `verification-smoke` が常時動く

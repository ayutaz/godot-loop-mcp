...
この文書は設計判断の調査記録です。  
2026-03-12 時点で `M0: Foundation and Contracts` の最小実装は repo に反映済みです。
...
実装済みの範囲は `Addon skeleton + TypeScript server skeleton + localhost TCP bridge + handshake + bidirectional ping + capability logging` です。  

...
2026-03-14 時点では `M0` から `M6` の実装面が repo に反映済みで、`M5` の code/workflow 側は実装済みです。残るのは npm 側の organization / scope 作成と trusted publisher 登録です。
...
| `M5` | 外部設定待ち | security level enforcement, `.godot/mcp/audit.log`, reusable GitHub Actions workflows, `verification-smoke`, release `test-reports` asset, publishable `packages/server`, Asset Library checklist を実装。残るのは npm organization / scope `godot-loop-mcp` の作成と npm trusted publisher の npm 側登録 |
...
- npm organization / scope `godot-loop-mcp` の作成

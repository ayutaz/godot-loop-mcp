...
2026-03-14 時点では `M0` から `M6` の実装面が repo に反映済みで、`M5` の code/workflow 側は実装済みです。残るのは npm organization `godot-loop-mcp` の作成と trusted publisher 登録です。

## 現在の実装状況

2026-03-13 時点の実装状態は次です。
...
| `M5` | 外部設定待ち | security level enforcement, `.godot/mcp/audit.log`, reusable GitHub Actions workflows, `verification-smoke`, release `test-reports` asset, publishable `packages/server`, Asset Library checklist を実装。残るのは npm organization `godot-loop-mcp` の作成と npm trusted publisher の npm 側登録 |
...
- bridge は `localhost` TCP + `4-byte big-endian length prefix + UTF-8 JSON`
- handshake は `hello -> hello response -> handshake.sync -> ready -> ping`
- 最短確認:
...
- 期待値: server log に `Addon hello accepted.`, `Addon handshake completed.`、addon log に `Bridge handshake completed.`, `Ping acknowledged.`
...
- `Godot 4.5+` は `OS.add_logger()` ring buffer を優先
- `Godot 4.4` は `.godot/mcp` log fallback
...
- security level は `WorkspaceWrite`
- headless の `play_scene` は別の Godot runtime process を起動
- runtime 出力は `.godot/mcp/runtime.log`
...
- pre-session では fallback log surface のみ
- handshake 後に capability-aware catalog が展開
- `text` 検索は `.godot`, `.git`, `node_modules` を除外
...
- `run_tests` adapter 優先順:
  1. env
  2. project settings
  3. GdUnit4
  4. GUT
- screenshot は GUI editor のみ
- `runtime.debug` は GUI editor + `runtime_telemetry.gd` autoload が必要

### M5

- security level:
  - server: `GODOT_LOOP_MCP_SECURITY_LEVEL`
  - addon: `GODOT_LOOP_MCP_SECURITY_LEVEL` または `godot_loop_mcp/security/level`
- dangerous opt-in:
  - `GODOT_LOOP_MCP_ENABLE_EDITOR_SCRIPT`
  - `GODOT_LOOP_MCP_ALLOWED_WRITE_PREFIXES`
  - `GODOT_LOOP_MCP_ALLOWED_SHELL_COMMANDS`
- audit log:
...
- 実装済み: reusable workflow, `verification-smoke`, release `test-reports` asset, publishable package 契約
- 外部設定待ち: npm organization `godot-loop-mcp` の作成と npm trusted publisher の npm 側登録
  - 対象 package: `@godot-loop-mcp/server`
...
- `os_shell` は allowlisted executable のみ
- `filesystem_write_raw` は allowlisted workspace prefix のみ
- `execute_editor_script` は explicit opt-in 前提
...
- bridge 契約とローカル確認の要約を `docs/implementation-milestones.md` に統合
...
- `reusable-server-check.yml` と `reusable-godot-smoke.yml` を追加し、`ci.yml`, `nightly-compat.yml`, `release.yml` の共通 job を整理
- `ci.yml` で `server-check`, `bridge-smoke`, `verification-smoke` を追加
- `nightly-compat.yml` で `windows-latest` / `ubuntu-latest` の nightly smoke matrix を追加
- `release.yml` で release smoke, `test-reports`, Addon ZIP, server tarball, `SHA256SUMS`, GitHub Release upload を追加
- `scripts/actions/` に Godot install, smoke, packaging, checksum scripts を追加
- `docs/asset-library-release-checklist.md` を追加
- `packages/server/package-lock.json`, publishable package metadata, CLI bin を追加

### 外部設定待ち

- npm organization `godot-loop-mcp` の作成
- npm trusted publisher の npm 側設定

### 完了条件

- 危険レベルに応じて tool 公開が変わる
- tool 呼び出しログに client 名、tool 名、引数ハッシュ、実行時間、失敗内容が残る
- PR で `server-check`, `bridge-smoke`, `verification-smoke` が回る
- nightly で Godot version matrix が回る
- `v*` tag で GitHub Release assets と `test-reports` が生成される
- server の npm publish が trusted publishing で `v*` tag push から実行できる
